...
/**
 * @author 程智凌
 * @date 2019/8/10 21:54
 */
public class CompressUtil2 {
    ...
    public static int long2bytes(List<Long> list, byte[] arr){
        ...
        for(int i = 1; i < list.size(); i++){
            ...
            if(delta == 0) {
                //0
                pos += 8;
            }else if(delta >= (-(1L << 6) + 1) && delta <= ((1L << 6) - 1)){
                //0
                pos ++;
                long2bits(arr, 7, pos, delta);
                pos += 7;
            }else if(delta >= (-(1L << 13) + 1) && delta <= ((1L << 13) - 1)){
                //10 14bit
                arr[pos >> 3] |= 0x01;
                pos += 2;
                long2bits(arr, 14, pos, delta);
                pos += 14;
            }else if(delta >= (-(1L << 20) + 1) && delta <= ((1L << 20) - 1)){
                //110 21bit
                arr[pos >> 3] |= 0x03;
                pos += 3;
                long2bits(arr, 21, pos, delta);
                pos += 21;
            }else if(delta >= (-(1L << 27) + 1) && delta <= ((1L << 27) - 1)){
                //1110 28bit
                arr[pos >> 3] |= 0x07;
                pos += 4;
                long2bits(arr, 28, pos, delta);
                pos += 28;
            }else if(delta >= (-(1L << 34) + 1) && delta <= ((1L << 34) - 1)){
                //1111 0 35bit
                arr[pos >> 3] |= 0x0f;
                pos += 5;
                long2bits(arr, 35, pos, delta);
                pos += 35;
            }else if(delta >= (-(1L << 41) + 1) && delta <= ((1L << 41) - 1)){
                //1111 10 42bit
                arr[pos >> 3] |= 0x1f;
                pos += 6;
                long2bits(arr, 42, pos, delta);
                pos += 42;
            }else if(delta >= (-(1L << 48) + 1) && delta <= ((1L << 48) - 1)){
                //1111 110 49bit
                arr[pos >> 3] |= 0x3f;
                pos += 7;
                long2bits(arr, 49, pos, delta);
                pos += 49;
            }else if(delta >= (-(1L << 55) + 1) && delta <= ((1L << 55) - 1)){
                //1111 1110 56bit
                arr[pos >> 3] |= 0x7f;
                pos += 8;
